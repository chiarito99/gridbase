...
if __name__ == "__main__":
    ...
    ox =     [67.59725046944169,-23.447229479586507,-37.898095633954156, 23.75455681577883,67.59725046944169]
    oy = [-21.759467298418425, -66.12037135288054, 74.93849874445242, 43.96162866522312, -21.759467298418425]
    resolution = 12.171085866445702
    ...
    ax.plot(map.ox, map.oy, np.ones(len(map.ox))*10, '-xk', label='range')
    ax.plot(map.traj[0,:], map.traj[1,:], 10, '-b', label='reference')
    # Zc=2
    # for i in range (len(map.obs)):
    ...
    # ax.plot(x_start, y_start, 10, 'ks', label='start')    # start
    # ax.plot(x_end, y_end, 10, 'ko', label='end')    # end
